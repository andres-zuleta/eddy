...
subroutine apply_matrix_R2(p, warp, twist, inc, PA, azi, pout)
   ...
   pout(1) = x*(sin(PA_)*sin(inc_)*sint*sinw - sin(PA_)*sint*cos( &
      inc_)*cosw + cos(PA_)*cost) + y*(sin(PA_)*sin(inc_) &
      *sinw*cost - sin(PA_)*cos(inc_)*cost*cosw - sint*cos(PA_)) + z*(sin(PA_)*sin(inc_)*cosw + sin(PA_)*sinw*cos(inc_))

   pout(2) = x*(sin(PA_)*cost - sin(inc_)*sint*sinw*cos(PA_) + &
      sint*cos(PA_)*cos(inc_)*cosw) + y*(-sin(PA_)*sint - sin(inc_)*sinw*cos(PA_)*cost + cos(PA_)* &
      cos(inc_)*cost*cosw) + z*(-sin(inc_)*cos(PA_)*cosw - sinw*cos(PA_)*cos(inc_))

   pout(3) = x*(sin(inc_)*sint*cosw + sint*sinw*cos(inc_ &
      )) + y*(sin(inc_)*cost*cosw + sinw*cos(inc_ &
      )*cost) + z*(-sin(inc_)*sinw + cos(inc_)*cosw)
...
subroutine apply_matrix2D_R2(p, warp, twist, inc, PA, azi, pout, nr, nphi)
...
do ir = 1, nr
   do iphi = 1, nphi
      call apply_matrix_R2(p(ir, iphi, :), warp(ir), phi_t(ir), inc_, PA_, azi_, pout(ir, iphi, :))
   end do
end do
...
subroutine apply_matrix_attila(p, warp, twist, inc, PA, azi, pout)
   ...
   pout(1) = x*(-sin(PA_)*sin(inc_)*sinw*cost - sin(PA_)*sint*cos &
      (inc_) + cos(PA_)*cost*cosw) + y*(sin(PA_)*sin(inc_ &
      )*sint*sinw - sin(PA_)*cos(inc_)*cost - sint*cos(PA_)*cosw) + z*(sin(PA_)*sin(inc_)*cosw + sinw*cos(PA_))

   pout(2) = x*(sin(PA_)*cost*cosw + sin(inc_)*sinw*cos(PA_)* &
      cost + sint*cos(PA_)*cos(inc_)) + y*(-sin(PA_)*sint*cosw - sin(inc_)*sint*sinw*cos(PA_) &
      + cos(PA_)*cos(inc_)*cost) + z*(sin(PA_)*sinw - sin(inc_)*cos(PA_)*cosw)

   pout(3) = x*(sin(inc_)*sint - sinw*cos(inc_)*cost) + y*(sin( &
      inc_)*cost + sint*sinw*cos(inc_)) + z*cos(inc_)*cosw
...
subroutine apply_matrix2D_attila(p, warp, twist, inc, PA, azi, pout, nr, nphi)
...
do ir = 1, nr
   do iphi = 1, nphi
      call apply_matrix_attila(p(ir, iphi, :), warp(ir), phi_t(ir), inc_, PA_, azi_, pout(ir, iphi, :))
   end do
end do
