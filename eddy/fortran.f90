...
! -------------------------------------------------------------------
   ...
   subroutine apply_matrix(p, warp, twist, inc, PA, azi, pout)
      ...
      *sin(inc_)*sinw) + z*(-(-sin(PA_)*cos(inc_)*cost - &
      sint*cos(PA_))*sinw + sin(PA_)*sin(inc_)*cosw)

      pout(2) = x*(sin(PA_)*cost + sint*cos(PA_)*cos(inc_)) + y*((-sin(PA_)* &
      sint + cos(PA_)*cos(inc_)*cost)*cosw - sin(inc_) &
      *sinw*cos(PA_)) + z*(-(-sin(PA_)*sint + cos(PA_)*cos( &
      inc_)*cost)*sinw - sin(inc_)*cos(PA_)*cosw)

      pout(3) = x*sin(inc_)*sint + y*(sin(inc_)*cost*cosw + sinw* &
      cos(inc_)) + z*(-sin(inc_)*sinw*cost + cos(inc_)*cosw)
